...
private:
    ...
    void openDatabase() {
        ...
        // Prevent base-level compactions from growing to the default 10x size
        // (which can produce ~400MB SSTs when target=40MB). Keep base level
        // bounded at ~2x the per-file target.
        opts.max_bytes_for_level_base = SST_TARGET_BYTES * 2;
        // Keep memtable flushes smaller than the target file size so individual
        // flushed SST files stay near the intended size.
        opts.write_buffer_size = SST_TARGET_BYTES / 2;
